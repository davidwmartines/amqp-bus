...
module.exports.subscribe = (bus, subscriptionRequest, cb) => {
  ...
  const consumeExchangeName = 'node_' + subscriptionRequest.messageType
    .replace(':', '.')
    .replace(/\./g, '_') + '_consumer_queue';

  const consumeQueueName = subscriptionRequest.roundRobinConsumer ? consumeExchangeName : consumeExchangeName + '_' + shortid.generate();
